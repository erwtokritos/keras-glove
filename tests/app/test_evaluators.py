...
import app.evaluators as ev


def test_most_similar(mocker):

    mock_np_load = mocker.patch('app.evaluators.np.load')
    ...
    mock_np_argsort = mocker.patch('app.evaluators.np.argsort')
    mock_np_argsort.return_value = np.array([0, 2, 1])

    mock_open = mocker.patch('app.evaluators.open')
    mock_open.return_value = 'test'

    mock_pickle_load = mocker.patch('app.evaluators.pickle.load')
